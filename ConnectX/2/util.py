...
def bitboard_to_train_state(bitboard, player_id):
    ...
    """
    layer1 = np.zeros((NUM_ROWS, NUM_COLUMNS))
    layer2 = np.zeros((NUM_ROWS, NUM_COLUMNS))
    layer3 = np.zeros((NUM_ROWS, NUM_COLUMNS))
...
def train_state_to_kaggle_state(train_state):
    """
    Train state is the 3x6x7 state used for training
...
def get_win_percentages_kaggle(agent1, agent2, n_rounds=10):
    ...
    return np.round(outcomes.count([1,-1])/len(outcomes), 4)
...
class ReplayMemory(object):
    ...
    def push_flipped(self, *args):
        ...
        action = args[1]
        flipped_action = NUM_COLUMNS - 1 - action
        next_state = args[2]
